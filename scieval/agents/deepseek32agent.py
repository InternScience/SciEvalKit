...
class Deepseek32Agent(AgentBase):
    name = "deepseek32agent"
    ...
    def run_sample(self, question, images = None):
        ...
        while num_llm_calls_available > 0:
            ...
            #             "role": message.role,
            #             "content": "<think>" + message.reasoning_content.strip() + "</think>" + message_content if message.reasoning_content else message_content,
            #             # "content": message.content.strip() if message.content else None,
            #             "tool_calls": [{"id": tool_call.id, "function": {"name": tool_call.function.name, "arguments": tool_call.function.arguments}} for tool_call in message.tool_calls] if hasattr(message, 'tool_calls') and message.tool_calls is not None else None,
            #         })
    ...
    def run(self, sample: EvalSample) -> EvalResult:
        if not self.api_key:
            raise ValueError("OPENAI_API_KEY is required for Deepseek32Agent.")
        if sample.images:
